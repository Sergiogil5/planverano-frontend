...
const PlayerFeedbackDetailView: React.FC<PlayerFeedbackDetailViewProps> = ({ player, onClose }) => {
  ...
  useEffect(() => {
    const fetchFeedback = async () => {
      ...
      setError(null);
      try {
        const feedback = await ApiService.getPlayerFeedback(player.id);
        setPlayerFeedback(feedback);
      } catch (err) {
        console.error(`Error fetching feedback for ${player.firstName}:`, err);
  ...
  const playerDisplayName = `${player.firstName} ${player.lastName}`;
  const playerTitle = player.accessCode 
    ? `${playerDisplayName} (${player.accessCode})` 
    : playerDisplayName;
        ...
        <header className="flex items-center justify-between mb-4 pb-3 border-b border-gray-700">
          <div className="flex-1 min-w-0"> {/* Added for truncation */}
            <h2 className="text-xl sm:text-2xl font-semibold text-purple-300 truncate" title={`Feedback de ${playerTitle} (${player.team})`}>
              Feedback de {player.firstName} {player.lastName} 
              {player.accessCode && <span className="text-base text-gray-400 ml-1">({player.accessCode})</span>}
              <span className="text-sm text-gray-400 ml-2">({player.team})</span>
            </h2>
          </div>
        ...
        {loadingFeedback ? (
          ...
          <div className="overflow-y-auto custom-scrollbar-details pr-2 flex-1">
            <ul className="space-y-4">
              {playerFeedback.map(fb => {
                const exercisesForDay = getDayExercises(fb.weekNumber, fb.dayName);
                return (
                  <li key={fb.id} className="bg-gray-700 p-4 rounded-md shadow-md">
                    <div className="flex justify-between items-start mb-2">
                      <span className="font-semibold text-lg text-purple-300">Semana {fb.weekNumber} - {fb.dayName}</span>
                    ...
                    {fb.activityLibreDetails && (
                      <div className="mt-2 pt-2 border-t border-gray-600">
                        <p className="text-sm text-gray-300">
                          <span className="font-semibold text-purple-200">Actividad Libre:</span> {fb.activityLibreDetails.activityType}
                          ({fb.activityLibreDetails.timeSpent})
                        </p>
                      </div>
                    )}
                    {exercisesForDay.length > 0 && (fb.exerciseActualDurations || fb.exerciseRoutes) && (
                      <div className="mt-3 pt-3 border-t border-gray-600">
                        ...
                        <ul className="space-y-1.5 text-xs">
                          {exercisesForDay.map((exercise, index) => {
                            const actualDurationSeconds = fb.exerciseActualDurations ? fb.exerciseActualDurations[index] : undefined;
                            const routeData = fb.exerciseRoutes ? fb.exerciseRoutes[index] : undefined;
                            const isTrackableRunningExercise = RUNNING_EXERCISES_FOR_TRACKING.includes(exercise.name.toLowerCase());

                            // Show item if it has duration, or route, or it's a non-timed rep based exercise (like "Flexiones: 10")
                            const shouldDisplayExerciseDetail = actualDurationSeconds !== undefined || 
                                                               (isTrackableRunningExercise && routeData && routeData.length > 0) ||
                                                               (!exercise.name.toLowerCase().includes("estiramientos") && exercise.repetitions !== "-");

                            if (!shouldDisplayExerciseDetail) return null;

                            return (
                              <li key={index} className="py-1.5 px-2 bg-gray-650 rounded">
                                <div className="flex justify-between items-center mb-1">
                                  <span className="text-gray-300 flex-1 mr-2">{exercise.name} <span className="text-gray-400 text-xxs">(Meta: {exercise.repetitions})</span></span>
                                  <span className={`font-mono ${actualDurationSeconds !== undefined ? 'text-green-300' : 'text-gray-500'}`}>
                                    {actualDurationSeconds !== undefined ? formatDuration(actualDurationSeconds) : 'N/A'}
                                  </span>
                                </div>
                                {isTrackableRunningExercise && routeData && routeData.length > 0 && (
                                 ...
                                 {isTrackableRunningExercise && (!routeData || routeData.length === 0) && (
                                  <p className="text-gray-500 text-xxs italic mt-0.5">No se grabó ruta GPS para este ejercicio.</p>
                                )}
                              </li>
                            );
                          })}
        ...
        >
          <div className="bg-gray-800 p-6 rounded-lg shadow-xl text-white max-w-sm w-full">
            ...
            <p id="delete-confirm-description" className="text-gray-300 mb-6">
              ¿Estás seguro que quieres eliminar a <strong>{player.firstName} {player.lastName}</strong> del plan de entrenamiento? Esta acción es irreversible.
            </p>
            {deleteError && (
